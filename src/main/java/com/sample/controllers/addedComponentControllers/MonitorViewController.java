...
public class MonitorViewController {
    ...
    private void deleteMonitor(){
        try{
            ...
            Monitor toBeDeleted = table.getSelectionModel().getSelectedItem();
    ...
    private void handleDeleteSucceed(WorkerStateEvent workerStateEvent) {
        try{
            ...
            if(table.getItems().toArray().length == 0){
                table.placeholderProperty().setValue(new Label("No Monitors saved"));
            }
        } catch (NullPointerException e){
    ...
    private void editDescription(TableColumn.CellEditEvent cellEditEvent){
        ...
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setDescription(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            Alert errorBox = new Alert(Alert.AlertType.ERROR);
            ...
            errorBox.showAndWait();
            selectedMonitor.setDescription(originalDescription);
            table.refresh();
        }
    }
    ...
    private void editPrice(TableColumn.CellEditEvent cellEditEvent){
        ...
        try{
            ...
            selectedMonitor.setPrice((double)cellEditEvent.getNewValue());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.showAndWait();
            selectedMonitor.setPrice(originalPrice);
            table.refresh();
        }
    }
    ...
    private void editName(TableColumn.CellEditEvent cellEditEvent) {
        ...
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File originalFile = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setProductName(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            ...
            File newFile = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
            if (newFile.equals(originalFile)){
                AdminLogic.editFile(originalFile, selectedMonitor);
            } else {
                //difference detected, change the filename before outputstreaming "new" object
                if (AdminLogic.editFileName(originalFile, newFile)){
                    AdminLogic.editFile(newFile, selectedMonitor);
                }
            }
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setProductName(originalName);
            table.refresh();
        }
    }
    ...
    private void editManufacturer(TableColumn.CellEditEvent cellEditEvent) {
        ...
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setProductionCompany(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            Alert errorBox = new Alert(Alert.AlertType.ERROR);
            errorBox.setHeaderText(e.getLocalizedMessage());
            selectedMonitor.setProductionCompany(originalManufacturer);
            errorBox.showAndWait();
            table.refresh();
        }
    }
    ...
    private void editSerialNumber(TableColumn.CellEditEvent cellEditEvent) {
        ...
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setSerialNumber(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setSerialNumber(originalSerialNumber);
            table.refresh();
        }
    }

    @FXML
    private void editDisplay(TableColumn.CellEditEvent cellEditEvent) {
        String originalDisplay = cellEditEvent.getOldValue().toString();
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setDisplayType(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setDisplayType(originalDisplay);
            table.refresh();
        }
    }
    @FXML
    private void editInches(TableColumn.CellEditEvent cellEditEvent) {
        String originalInches = cellEditEvent.getOldValue().toString();
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setInches(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setInches(originalInches);
            table.refresh();
        }
    }
    @FXML
    private void editResolution(TableColumn.CellEditEvent cellEditEvent) {
        String originalResolution = cellEditEvent.getOldValue().toString();
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setResolution(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setResolution(originalResolution);
            table.refresh();
        }
    }
    @FXML
    private void editConnector(TableColumn.CellEditEvent cellEditEvent) {
        String originalConnector = cellEditEvent.getOldValue().toString();
        Monitor selectedMonitor = (Monitor)cellEditEvent.getRowValue();
        File file = new File("src/main/java/com/sample/DAL/SavedFiles/NewComponents/Monitors/"+selectedMonitor.getProductName()+".jobj");
        try{
            selectedMonitor.setConnector(cellEditEvent.getNewValue().toString());
            selectedMonitor.validate();
            AdminLogic.editFile(file, selectedMonitor);
            table.refresh();
        } catch (IOException e){
            ...
            errorBox.setHeaderText(e.getLocalizedMessage());
            errorBox.showAndWait();
            selectedMonitor.setConnector(originalConnector);
            table.refresh();
        }
    }
}
